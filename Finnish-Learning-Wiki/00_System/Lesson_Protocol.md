...
Во время каждого `FULL_LESSON`, помимо основного priority retrieval, периодически проверять **1–3 ранее изученных chunks/patterns**, которые не являются главным фокусом текущего урока.

Цель — проверить **спонтанный recall вне недавно отработанного материала**, а не просто повторить то, что только что тренировалось.
...
CONSOLIDATING → чаще
STABLE → иногда
DORMANT → редко, без предупреждения
```

Не сообщать заранее, какой pattern или правило проверяется. Задача должна требовать самостоятельного распознавания и использования формы по смыслу и контексту.

Результат sampling классифицировать как:
...
Sampling может влиять на последующие mastery/retention решения. Повторение того же target много раз внутри основного урока не считать независимым sampling evidence.

Не проверять один и тот же sampled item в каждом последовательном уроке; sampling должен оставаться действительно неожиданной выборкой.
